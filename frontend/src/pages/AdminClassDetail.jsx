              ...
              <div>
                ...
                <div className="flex items-center gap-3 mt-1">
                  <span className={`text-sm font-semibold ${classData?.availableSeats <= 0 ? 'text-red-500' : 'text-green-600'}`}>
                    {booked}/{classData?.totalSeats} rezervirano
                  </span>
                  <span className="text-dark-400 text-sm">
                    ({classData?.availableSeats} slobodnih)
                  </span>
                </div>
              </div>
            </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-dark-200">
                    <th className="text-left py-3 px-4 text-dark-500 font-semibold">#</th>
                    <th className="text-left py-3 px-4 text-dark-500 font-semibold">Ime i prezime</th>
                    <th className="text-left py-3 px-4 text-dark-500 font-semibold">Email</th>
                    <th className="text-left py-3 px-4 text-dark-500 font-semibold">Telefon</th>
                    <th className="text-left py-3 px-4 text-dark-500 font-semibold">Datum rezervacije</th>
                    <th className="text-left py-3 px-4 text-dark-500 font-semibold">Napomena</th>
                    <th className="text-right py-3 px-4 text-dark-500 font-semibold">Akcija</th>
                  </tr>
                </thead>
                <tbody>
                  {bookings.map((booking, index) => (
                    <tr key={booking.bookingId} className="border-b border-dark-100 hover:bg-dark-50 transition-colors">
                      ...
                      <td className="py-3 px-4 text-dark-500">{formatDate(booking.bookedAt)}</td>
                      <td className="py-3 px-4 text-dark-500 max-w-[200px] truncate">{booking.notes || '-'}</td>
                      <td className="py-3 px-4 text-right">
                        ...
                        >
                          {cancellingId === booking.bookingId ? 'Otkazivanje...' : 'Otkaži'}
                        </button>
                      </td>
                    </tr>
                  ))}
