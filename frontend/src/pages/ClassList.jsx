...
export default function ClassList() {
  ...
  const loadClasses = async () => {
    try {
      ...
      setClasses(items)
      // Fetch reviews for all classes
      items.forEach((cls) => {
        publicApi.getClassReviews(cls.classId)
          .then((rev) => setReviewsData((prev) => ({ ...prev, [cls.classId]: rev })))
          .catch(() => {})
      })
    } catch (err) {
              ...
              return (
                ...
                >
                  ...
                  {reviews && reviews.totalReviews > 0 && (
                    <div className="mb-5 rounded-xl border border-dark-100 overflow-hidden">
                      <button
                        onClick={() => setExpandedReviews((prev) => ({ ...prev, [cls.classId]: !prev[cls.classId] }))}
                        className="w-full flex items-center justify-between p-3 bg-dark-50 hover:bg-dark-100 transition-colors cursor-pointer"
                      >
                        <div className="flex items-center gap-2">
                          <div className="flex items-center gap-0.5">
                            {[1, 2, 3, 4, 5].map((star) => (
                              <svg
                                key={star}
                                className={`w-4 h-4 ${star <= Math.round(reviews.averageScore) ? 'text-yellow-400' : 'text-dark-200'}`}
                                fill="currentColor"
                                viewBox="0 0 20 20"
                              >
                          ...
                          <span className="text-sm font-semibold text-dark-700">{reviews.averageScore.toFixed(1)}</span>
                          <span className="text-xs text-dark-400">({reviews.totalReviews} {reviews.totalReviews === 1 ? 'recenzija' : 'recenzija'})</span>
                        </div>
                        <svg className={`w-4 h-4 text-dark-400 transition-transform ${expandedReviews[cls.classId] ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                        </svg>
                      </button>
                      {expandedReviews[cls.classId] && (
                        <div className="p-3 space-y-2 border-t border-dark-100 max-h-[200px] overflow-y-auto">
                          {reviews.reviews.map((review, idx) => (
                            <div key={idx} className="bg-white rounded-lg p-2.5 border border-dark-100">
                              <div className="flex items-center gap-1 mb-1">
                                <div className="flex items-center gap-0.5">
                                  {[1, 2, 3, 4, 5].map((star) => (
                                    <svg
                                      key={star}
                                      className={`w-3.5 h-3.5 ${star <= review.score ? 'text-yellow-400' : 'text-dark-200'}`}
                                      fill="currentColor"
                                      viewBox="0 0 20 20"
                                    >
                                      <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                                ...
                                <span className="text-[0.65rem] text-dark-400 ml-auto">
                                  {new Date(review.submittedAt).toLocaleDateString('hr-HR', { day: 'numeric', month: 'long', year: 'numeric' })}
                                </span>
                              </div>
                              <p className="text-xs text-dark-600 line-clamp-2">{review.comment}</p>
                            </div>
                          ))}
                        </div>
                      )}
